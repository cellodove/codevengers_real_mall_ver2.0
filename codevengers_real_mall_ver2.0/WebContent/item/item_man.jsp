...
<body>
	<h1>남자 신발</h1>
	남자신발: &nbsp;<strong><c:out value="${listcount}" /></strong>개
	<table>
					...
					<tbody>
						...
						<tr>
							<td colspan="5"><c:choose>
									...
									<c:otherwise>
										<a href="./ItemMan.no?page=<c:out value="${page-1}"/>">[이전]</a>&nbsp;
									</c:otherwise>
									...
									<c:choose>
										...
										<c:otherwise>
											<a href="./ItemMan.no?page=<c:out value="${start}"/>">[<c:out
													value="${start}" />]
											</a>&nbsp;
										</c:otherwise>
									</c:choose>
									...
									<c:otherwise>
										<a href="./ItemMan.no?page=<c:out value="${page+1}"/>">[다음]</a>
									</c:otherwise>
								</c:choose></td>
						</tr>
					</tbody>
